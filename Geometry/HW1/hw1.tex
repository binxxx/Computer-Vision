% Homework-1 for Fall-2017. Thanks to Abhinav Shrivastava and Nick Rhinehart!
% (previous years TAs)
...
\def\w{\mathbf{w}}
\def\K{\mathbf{K}}
\lecture{{\bf 16-822}: Geometry-based Methods in Vision (F17) }{Released: Sep 13. Due: Oct 04. }{Lecturer: Martial Hebert}{TA: Aayush Bansal}{Homework 1}

\section{Theory (55 Points)}
...
\subsection{Center of a virtual camera (image of an image); (10 Points) }
Let $I_1$ be an image of a scene, using a general projective model. Let $I_2$ be the image obtained by seeing through another camera (an image of an image). We can think of $I_2$ as an image of the original scene through some virtual camera. Show that the center $C$ of this virtual camera is the same as the center of the projection used for $I_1$.
